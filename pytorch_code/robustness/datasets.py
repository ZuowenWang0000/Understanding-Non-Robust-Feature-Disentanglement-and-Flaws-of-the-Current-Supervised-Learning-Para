...
# from custom_torchvision_datasets

from .tools import constants
from .tools.folder import TensorDataset
...
class constructed_CIFAR(DataSet):
    """
    constructed CIFAR-10 dataset from Madry lab
    ...
    def __init__(self, data_path='/tmp/', **kwargs):
        ...
        super(constructed_CIFAR, self).__init__('constructed_cifar', data_path, **ds_kwargs)
...
class MixedSampler(Sampler):
    """
    Custom sampler to sample a minibatch of robust and non-robust images. Operates on data
    sources of type RobustCIFARDataset. batch_size specifies the size of the batch that each
    sample from this sampler returns. One epoch is considered complete once 50'000 (size of 
    natural dataset) images have been seen. batch_ratio is the proportion of robust images 
    making up the batch (0 = no robust images, 1 = only robust images).
    """
    def __init__(self, data_source, batch_size, batch_ratio):
        self.data_source = data_source
        self.batch_size = batch_size
        self.batch_ratio = batch_ratio
        self.random_sampler_robust = iter(RandomSampler(data_source.train_set_robust))
        self.random_sampler_natural = iter(RandomSampler(data_source.train_set_natural))

    def __iter__(self):
        for _ in range(math.ceil(len(self.data_source.train_set_natural) / self.batch_size)):
            batch = []
            # Try to fill the batch with the appropriate balance of robust and natural
            for _ in range(math.floor((1-self.batch_ratio) * self.batch_size)):
                try:
                    batch.append(next(self.random_sampler_natural))
                except StopIteration:  # This means that we are at the end and there are not enough images to fill the batch
                    break
            for _ in range(math.ceil(self.batch_ratio * self.batch_size)):
                try:
                    batch.append(next(self.random_sampler_robust) + len(self.data_source.train_set_natural))
                except StopIteration:  # This means that we are at the end and there are not enough images to fill the batch
                    break
            # assert len(batch) == self.batch_size
            # print(batch)
            yield batch
        self.random_sampler_robust = iter(RandomSampler(self.data_source.train_set_robust))
        self.random_sampler_natural = iter(RandomSampler(self.data_source.train_set_natural))
        # print("Reached end of control flow.")

    def __len__(self):
        # This is the amount of iterations per epoch
        return math.ceil(len(self.data_source.train_set_natural) / self.batch_size)

class RobustCIFARDataset(Dataset):
    def __init__(self, root, train, download, transform=None):
        self.train = train
        if train:
            train_data = ch.cat(ch.load(os.path.join(root, f"d_robust_CIFAR/CIFAR_ims")))
            train_labels = ch.cat(ch.load(os.path.join(root, f"d_robust_CIFAR/CIFAR_lab")))
            self.train_set_robust = TensorDataset(train_data, train_labels, transform=transform)
            self.train_set_natural = datasets.CIFAR10(root=root, train=train, download=download, transform=transform)
            # print(self.train_set_robust[3])
            # print(self.train_set_natural[3])
        else:
            self.test_set = datasets.CIFAR10(root=root, train=train, download=download, transform=transform)
    
    def __getitem__(self, index):
        if self.train:
            if index < len(self.train_set_natural):
                return self.train_set_natural[index]
            else:
                return self.train_set_robust[index - len(self.train_set_natural)]
        else:
            return self.test_set[index]
    
    def __len__(self):
        if self.train:
            # return len(self.train_set_natural)
            raise NotImplementedError  # No idea what the mix between robust and non-robust will be
        else:
...
class robust_CIFAR(DataSet):
    def __init__(self, data_path, **kwargs):
        ds_kwargs = {
            ...
            'mean': ch.tensor([0.4914, 0.4822, 0.4465]),
            'std': ch.tensor([0.2023, 0.1994, 0.2010]),
            'custom_class': RobustCIFARDataset,
            'label_mapping': None, 
            'transform_train': da.TRAIN_TRANSFORMS_DEFAULT(32),
            'transform_test': da.TEST_TRANSFORMS_DEFAULT(32)
        }
        super(robust_CIFAR, self).__init__('robust_cifar', data_path, **ds_kwargs)

    def get_model(self, arch, pretrained):
        """
        """
        if pretrained:
            raise ValueError('robust_CIFAR does not support pytorch_pretrained=True')
        return cifar_models.__dict__[arch](num_classes=self.num_classes)

    def make_loaders(self, workers, batch_size, data_aug=True, subset=None, 
                 subset_start=0, subset_type='rand', val_batch_size=None,
                 only_val=False, shuffle_train=True, shuffle_val=True, batch_ratio=1):
        if val_batch_size is None:
            val_batch_size = batch_size
        if not only_val:
            train_set = self.custom_class(root=self.data_path, train=True, 
                                        download=True, transform=self.transform_train)
            train_loader = DataLoader(train_set, batch_sampler=MixedSampler(train_set, batch_size, batch_ratio), 
                                        num_workers=workers, pin_memory=True)
        test_set = self.custom_class(root=self.data_path, train=False, 
...
class da_robust_CIFAR(DataSet):
    def __init__(self, data_path, **kwargs):
        ds_kwargs = {
            ...
            'custom_class': RobustCIFARDataset,
            'label_mapping': None, 
            'transform_train': da.TRAIN_TRANSFORMS_DEFAULT_STAR(32),
            'transform_test': da.TEST_TRANSFORMS_DEFAULT(32)
        }
        super(da_robust_CIFAR, self).__init__('da_robust_cifar', data_path, **ds_kwargs)

    def get_model(self, arch, pretrained):
        """
        """
        if pretrained:
            raise ValueError('da_robust_CIFAR does not support pytorch_pretrained=True')
        return cifar_models.__dict__[arch](num_classes=self.num_classes)
...
# TODO
#  REDUNDANCY HERE need to refactor the code before submission

class NonRobustCIFARDataset(Dataset):
    def __init__(self, root, train, download, transform=None):
        self.train = train
        if train:
            train_data = ch.cat(ch.load(os.path.join(root, f"d_non_robust_CIFAR/CIFAR_ims")))
            train_labels = ch.cat(ch.load(os.path.join(root, f"d_non_robust_CIFAR/CIFAR_lab")))
            self.train_set_robust = TensorDataset(train_data, train_labels, transform=transform)
            self.train_set_natural = datasets.CIFAR10(root=root, train=train, download=download, transform=transform)
            # print(self.train_set_robust[3])
            # print(self.train_set_natural[3])
...
class non_robust_CIFAR(DataSet):
    def __init__(self, data_path, **kwargs):
        ds_kwargs = {
            ...
            'mean': ch.tensor([0.4914, 0.4822, 0.4465]),
            'std': ch.tensor([0.2023, 0.1994, 0.2010]),
            'custom_class': NonRobustCIFARDataset,
            'label_mapping': None,
            'transform_train': da.TRAIN_TRANSFORMS_DEFAULT(32),
            'transform_test': da.TEST_TRANSFORMS_DEFAULT(32)
        }
        super(non_robust_CIFAR, self).__init__('robust_cifar', data_path, **ds_kwargs)
