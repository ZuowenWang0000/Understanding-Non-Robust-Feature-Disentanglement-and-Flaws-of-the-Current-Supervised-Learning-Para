...
STEPS = {
    ...
    'spatial_random': attack_steps.Spatial,
    'spatial_grid': attack_steps.Spatial
}
...
class Attacker(ch.nn.Module):
    ...
    def forward(self, x, target, *_, constraint, eps, step_size, iterations,
        ...
        is_spatial = False
        if step_class == attack_steps.Spatial:
            is_spatial = True
            attack_type = 'grid' if isinstance(constraint, str) and constraint == 'spatial_grid' else 'random'
            step = step_class(attack_type=attack_type, eps=eps, granularity=step_size)
        else:
